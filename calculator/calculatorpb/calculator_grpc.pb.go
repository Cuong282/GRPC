...
const (
	...
	Calculator_PrimeNumberDecomposition_FullMethodName = "/calculator.Calculator/PrimeNumberDecomposition"
)
...
func (c *calculatorClient) PrimeNumberDecomposition(ctx context.Context, in *PNDRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[PNDResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Calculator_ServiceDesc.Streams[0], Calculator_PrimeNumberDecomposition_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Calculator_PrimeNumberDecompositionClient = grpc.ServerStreamingClient[PNDResponse]
...
func (UnimplementedCalculatorServer) PrimeNumberDecomposition(*PNDRequest, grpc.ServerStreamingServer[PNDResponse]) error {
...
func _Calculator_PrimeNumberDecomposition_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(CalculatorServer).PrimeNumberDecomposition(m, &grpc.GenericServerStream[PNDRequest, PNDResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Calculator_PrimeNumberDecompositionServer = grpc.ServerStreamingServer[PNDResponse]
